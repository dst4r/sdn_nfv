...
mkdir -p j2
wget -q https://labs.alta3.com/courses/sd-wan/scripts/wg-basic/charlie.conf.j2 -O j2/charlie.conf.j2
mkdir -p conf
...
ssh charlie sudo wg setconf wg0 /etc/wireguard/wg0.conf
ssh charlie sudo ip -4 address add 10.66.0.1/32 dev wg0
ssh charlie sudo ip link set mtu 1500 up dev wg0 
ssh charlie sudo ip -4 route add 10.64.0.0/16 dev wg0
ssh charlie sudo ip -4 route add 10.65.0.0/16 dev wg0
ssh charlie sudo ip -4 route add 10.67.0.1/32 dev wg0
